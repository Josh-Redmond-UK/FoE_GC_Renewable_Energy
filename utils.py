...
def getExclusionsDict():
    ...
    "Roads": ee.FeatureCollection('projects/data-sunlight-311713/assets/UK_Roads_Buffer_200m').map(lambda x: x.buffer(50)).reduceToImage(properties= ['FEATCODE'], reducer= ee.Reducer.first()).unmask().lt(1),
    ...
    "Cycle Paths Wind": ee.FeatureCollection('projects/data-sunlight-311713/assets/cyclenet').map(lambda x: x.buffer(100)).reduceToImage(properties= ['FID'], reducer= ee.Reducer.first()).unmask().lt(1),
    "Cycle Paths Solar": ee.FeatureCollection('projects/data-sunlight-311713/assets/cyclenet').reduceToImage(properties= ['FID'], reducer= ee.Reducer.first()).unmask().lt(1),
    ...
    "England Cultural Sites": ee.FeatureCollection('projects/data-sunlight-311713/assets/england_culturalsites').reduceToImage(properties = ['ListEntry'], reducer = ee.Reducer.first()).unmask().lt(1),
    "Parks and Green Space": ee.FeatureCollection("projects/data-sunlight-311713/assets/GB_GreenspaceSite").reduceToImage(properties= ['areaHa'], reducer= ee.Reducer.first()).unmask().lt(1),
    ...
    "Built Up Areas": get_build_up_area_buffer(250).unmask().lt(1),
    ...
    'Scotland Cultural Sites': ee.FeatureCollection('projects/data-sunlight-311713/assets/ScotlandCulturalSites').map(lambda x: x.buffer(250)).map(lambda x: x.set('dummy', 1)).reduceToImage(properties= ['dummy'], reducer= ee.Reducer.first()).unmask().lt(1),
    'Scotland Agricultural Land': ee.FeatureCollection('projects/data-sunlight-311713/assets/ScotlandAgriculturalLand').reduceToImage(properties= ['LCCODE '], reducer= ee.Reducer.first()).unmask().lte(2),
    ...
    "England Agricultural Land": ee.FeatureCollection('projects/data-sunlight-311713/assets/AgriculturalLandEngland').reduceToImage(properties= ['hectares'], reducer= ee.Reducer.first()).unmask().lt(1)
    }
...
def getExclusions(type="common"):
    ...
    "Cycle Paths Wind",
    "Surface Water",
    "England Cultural Sites",
    "Scotland Cultural Sites",
